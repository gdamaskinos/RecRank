...
def getFeatures(graphPath, verbose=False):
  """Derives manual features from graph using graph_tool
  https://graph-tool.skewed.de/static/doc/graph_tool.html#available-subpackages
  Args:
    graphPath (str): path to .xml file
  Returns:
    list: features
    list: names
  """

  g = graph_tool.load_graph(graphPath)

  x = [] # input features
  names = [] # feature names

  # number of vertices
  x.append(g.num_vertices())
  names.append("vertices")
  if verbose: print("Vertices: ", g.num_vertices())

  # number of edges
  x.append(g.num_edges())
  names.append("edges")
  if verbose: print("Edges: ", g.num_edges())

  # vertex in-degree
  # TODO define bins edges i.e., x-axis positions => len(counts) = len(bins) - 1
  edges = range(0, 13)
  edges.append(g.num_edges())
  counts, bins = vertex_hist(g, "in", bins=edges)
  if verbose:
  ...
  avg, std = vertex_average(g, "in")
  if verbose: print("In-degree: ", vertex_average(g, "in"))
  x.append(avg)
  ...
  ls = []
  nanCount = 0
  for v in pagerank(g, weight=g.edge_properties["weight"]):
    if (math.isnan(v)):
      nanCount += 1
    else:
      ls.append(v)
...
#  names.append("page rank")
  ...
  temp = Graph(g)
  temp.set_directed(False)
  i = 0
  nanCount = 0
  ls = []
  for v in eigenvector(temp, weight=temp.edge_properties["weight"], \
      max_iter=10000)[1]:
    if (math.isnan(v)):
      nanCount += 1
    else:
...
#  names.append("eigenvector")
  ...
  nanCount = 0
  ls = []
  for v in betweenness(temp, weight=temp.edge_properties["weight"])[0]:
    if (math.isnan(v)):
      nanCount += 1
    else:
      ls.append(v)
...
#  names.append("betweeness")
  ...
  nanCount = 0
  ls = []
  for v in closeness(temp, weight=temp.edge_properties["weight"]):
    if (math.isnan(v)):
      nanCount += 1
    else:
      ls.append(v)
...
#  i = 0
#  ls = []
#  for v in katz(temp, weight=temp.edge_properties["weight"]):
#    ls.append(v)
##    print(g.vertex_properties["text"][i], v)
#    i += 1
#
...
#    rank += 1
  ...
  temp = Graph(g)
  temp.set_directed(False)
  assort, variance = assortativity(temp, "out")
  if verbose: print("(assortativity, variance): ", assortativity(temp, "out"))
  x.append(assort)
  ...
  for v in shortest_distance(temp, weights=temp.edge_properties["weight"], directed=False):
...
def manualDist(folder_path, extra=None, cmpRank_path=None, \
  ...
  Args:
    folder_path (str): folder path with N .xml graph files
    cmpRank_path (str): see main#args.cmpRank_path
    extra (str): extra .xml graph file (usefull for black-box)
  ...
  try:
    id1 = labels.index('149.xml')
    id2 = labels.index('bb.xml')
    comp.append(id1)
    comp.append(id2)
...
def kernelDist(folder_path, cmpRank_path=None, extra=None):
  ...
  if not extra is None and extra not in filenames:
    filenames.append(extra)
  for filename in set(filenames):
    if filename.endswith(".xml") or filename.endswith(".graphml"):
      indices.append(idx)
      idx += 1
