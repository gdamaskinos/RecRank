...
def librecParallelTopNComputation(testSet, librec_home, confPath, tmpdir, \
    N, update_freq, total, jobID):
  """
  Computes the topN recommendation for a range depending on the jobID for librec
  Args:
    tmpdir (string): temporal directory
    total (int): total number of processes
    jobID (int): process id.
      Defines which testSet rows (i.e., clicks) to derive the topN for
    See librec_recom() for the rest Args
  ...
  # write testSet
  temp_start = start - (start % update_freq) # train the model for the first loop

  testSet.iloc[temp_start+1:].to_csv(tmpdir + "/testSet" + str(jobID) + ".csv", \
  ...
  for i in range(start, end):
    ...
    # remove clicks from testSet (<=> add to the trainSet)
    if i % update_freq == 0:
#      cmd = ("sed -i -e 1d " + tmpdir + "/testSet.csv" )
#      process = subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
#      output, error = process.communicate()

      # if i+1==len(testSet) the testSet.csv will be empty <- librec works fine
      testSet.iloc[i+1:].to_csv(tmpdir + "/testSet" + str(jobID) + ".csv", \
          header=False, index=False)

    # train
    if i % update_freq == 0 or i == start:
      print("Training...")
      # output recommendation file with librec
      cmd = ("bash " + librec_home + "/bin/librec rec -exec -conf " + confPath + \
             " -jobconf dfs.result.dir=" + tmpdir + "/output" + str(jobID) + \
             " -jobconf dfs.data.dir=" + tmpdir + \
             " -jobconf data.input.path=fullData.csv" + \
             " -jobconf data.testset.path=testSet" + str(jobID) + ".csv" + \
             " -jobconf data.column.format=UIRT" +\
             " -jobconf data.model.splitter=testset" + \
             " -jobconf rec.random.seed=42 " + \
             " -jobconf rec.eval.enable=true" + \
             " -jobconf rec.recommender.isranking=true" + \
             " -jobconf rec.recommender.ranking.topn=" + str(N) + \
             " -libjars " + librec_home + "/lib/log4j-1.2.17.jar")
      # redirect STDOUT to python stderr
      process = subprocess.Popen(cmd.split(), stderr=subprocess.STDOUT, stdout=subprocess.PIPE)
      #process = subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
      output, error = process.communicate()

      try:
        outputPath = glob.glob(tmpdir + "/output" + str(jobID) + "/fullData.csv-*/*")[0]
      except IndexError:
        print("Librec training failed. Run manually to investigate:\n" + cmd)
        raise IndexError()

      # get predictions for all the items in the testSet
      preds = pd.read_csv(outputPath, header=None, \
          names=['user_id', 'item_id', 'prediction'], \
          dtype={'user_id': str, 'item_id': str})

    # get user clicked items in the test set
    #   to avoid recommending clicked items if update_freq > 0
    temp_clicked = testSet.iloc[:i+1]
    temp_clicked = temp_clicked[temp_clicked['user_id'] == click['user_id']]
    temp_clicked = set(temp_clicked['item_id'])

    # get topN recommendations for the user
    recoms = preds[(preds['user_id'] == click['user_id']) \
        & (~preds['item_id'].isin(temp_clicked))] \
        .sort_values('prediction', ascending=False)

    top = []
    for idx, recom in recoms.iterrows():
      if recom['prediction'] < 0:
        print("Rounding to 0 negative prediction:\n%s" % recom)
        recom['prediction'] = 0
      top.append((recom['item_id'], recom['prediction']))

    print("JobID:%d\nTopN for click %d: [%s, %s, %s, %s]\n%s\nRemaining clicks: %d" % (
      jobID, i, click['user_id'], click['item_id'], click['rating'], \
      click['timestamp'], top, end - 1 - i))

#    tsize = len(pd.read_csv(tmpdir + "/testSet"+ str(jobID) + ".csv", header=None, \
#        names=['user_id', 'item_id', 'rating', 'timestamp'], \
#                      dtype={'user_id': str, 'item_id': str}))
#    print("HOLA%d: %d %d %d" % (jobID, temp_start, i, tsize))
...
def librec_recom(trainSet, testSet, librec_home, confPath, N_list=[5], \
    update_freq=1, num=multiprocessing.cpu_count(), evalTrain=True):
  """Get topN recommendations for every click in the test set using LibRec

  Args:
    trainSet (pandas.DataFrame): training dataset with user_id, item_id, rating, timestamp
    testSet (pandas.DataFrame): test dataset with user_id, item_id, rating, timestamp
    librec_home (string): *absolute* path to the cloned librec repo
      git clone https://github.com/guoguibing/librec.git
    confPath (string): *absolute* path to the .properties file containing
      algorithmic specific configurations (e.g., number of factors)
      Any given config that exists in this function (e.g., data.column.format)
      will be ignored.
    N_list (list): top-N = max(N_list)
      the rest of the N-values are only used for the evaluation
    update_freq (int): after how many clicks to update the model
  ...
  """
  # print configuration
  print("Training size: %d" % len(trainSet))
  print("Test size: %d" % len(testSet))
  print("N_list: %s" % N_list)
  print("update_freq: %d" % update_freq)
  print("Eval Train: %s" % evalTrain)
  print("Algorithm properties: ")
  with open(confPath, 'r') as fin:
    print(fin.read())
  s_time = time.time()
  print("Start time: %s" % s_time)

  filePath = os.path.abspath(os.path.join(os.path.realpath(__file__), os.pardir))
  ...
  # write fullDataset by concatenating train and test
  trainSet.append(testSet).to_csv(
      tmpdir + '/fullData.csv', header=False, index=False)

  start = time.time()
  testSet.to_csv(tmpdir + '/testSet.csv', header=False, index=False) # write testSet

  # Evaluate recommendation algorithm
  if evalTrain:
    print("Evaluating recommendation algorithm with 5-fold KV on train \\union test")
    cmd = ("bash " + librec_home + "/bin/librec rec -exec -conf " + confPath + \
           " -jobconf dfs.result.dir=" + tmpdir + "/output" + \
           " -jobconf dfs.data.dir=" + tmpdir + \
           " -jobconf data.input.path=fullData.csv" + \
           " -jobconf data.column.format=UIRT" + \
           " -jobconf data.model.splitter=kcv" + \
           " -jobconf rec.parallel.support=true" + \
           " -jobconf data.splitter.cv.number=5" + \
           " -jobconf rec.random.seed=1 " + \
           " -jobconf rec.eval.enable=true" + \
           " -jobconf rec.recommender.isranking=false" + \
           " -libjars " + librec_home + "/lib/log4j-1.2.17.jar")
    process = subprocess.Popen(cmd.split(), stdout=subprocess.PIPE)
    output, error = process.communicate()
  ...
  cmd = ("bash " + librec_home + "/bin/librec rec -exec -conf " + confPath + \
         ...
         " -jobconf dfs.data.dir=" + tmpdir + \
         " -jobconf data.input.path=fullData.csv" + \
         " -jobconf data.testset.path=testSet.csv" + \
         " -jobconf data.column.format=UIRT" +\
         " -jobconf data.model.splitter=testset" + \
         " -jobconf rec.random.seed=42 " + \
         " -jobconf rec.eval.enable=true" + \
  ...
  estimate = len(testSet) * (time.time() - start) / float(num)
  print("Execution time per training: %.2f sec" % (time.time() - start))
  print("Estimated execution time: %.2f sec" % estimate)
  # Parallel topN computation
  part = partial(librecParallelTopNComputation, testSet, \
      librec_home, confPath, tmpdir, max(N_list), update_freq, num)
  jobIDs = range(0, num)
  print("Spawning %d processes for topN computation..." % num)
  pool = multiprocessing.Pool(num)
  outputs = pool.map_async(part, jobIDs).get(9999999) # enable killing with SIGTERM
  pool.close()
  pool.join()
